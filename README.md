...
### 0522 考察知识点：数组的操作 hashMap 的映射 （leetcode 第一题 非常经典）
...
### 改变函数内部this指针的方式有哪几种，他们的区别是什么？
1、通过apply和call改变函数的this指向，他们两个函数的第一个参数都是一样的表示要改变指向的那个对象。
2、传参的方式 apply 传递参数是以数组的形式  call 是按照 argu1、argu2 这种形式传递。
3、bind 方法返回一个函数，称之为绑定函数 第一个参数 是想要绑定的this指向，第二个以及后面的参数作为参数传递。
```js
var bar = function(){ 
  console.log(this.x);   
}
...
首先函数传参是传递对象指针的副本，就拿这个例子来说,p1传递进去，在内部person是形参，修改了person的值，将person的age属性修改成了26，当然对应的p1的值也会发生改变，但是 当我们在函数内部重新给person赋值了一个对象的时候, 出现了分歧, 实际上这个时候 被重新赋值的这个person和原本传递进去的p1已经没有任何的关系了。

这个题目考察的其实是函数参数是对象的情况。
...
下面这个是常见的一种判断对象具体类型的场景:
```js
  // 定义一个构造函数 Person
  const Person = function() {}
  // 通过Person new 出来一个实例 p1
  const p1 = new Person()
  // 使用 instanceof 判断 是否是具体对象的具体类型
  console.log(p1 instanceof Person) // true
```

对于原始类型来说，直接使用instanceof是不行的,请看下面这个例子：
...
### 四则运算符:
1、加法运算符 不同于其他几个运算符它有几个特点
  * 运算中一方为字符串，另一方也会装换为字符串
  * 如果一方不是字符串或者数字，会将他们转换为字符串或者数字
```js
1+'1' = '11'
true + true = 2
4+[1,2,3] = "41,2,3"
```
对于第一行代码来说, 触发特点一：所以将数字 1 转换成 字符串 '1' 得到结果 '11'
对于第二行代码来说, 触发特点二：所以将true转换为数字。
对于第三行代码来说，触发特点二：所以将数组通过 toString() 方法转换为 1，2，3得到结果 "41,2,3"
...
最后 类似于 `bind` 这种改变上下文的api 对于这个函数来说，this 取决于第一个参数，如果第一个参数为空，那么就是window

最后: this的指向问题有可能出现多个规则同时出现的情况,这个时候会根据优先级来判断this最终指向哪里:

首先 new 的优先级最高，接下来是 bind 这种api 然后是 obj.foo() 这种形式，最后是 foo() 这种形式，同时箭头函数的this 一旦绑定，
就不会被任何方式改变。
...
1、== 两等 等同 是比较运算符 两遍值类型不同的时候，先进行类型转化，再比较
2、=== 恒等 严格比较运算符，不做类型转换，类型不同就是不等
3、Object.is() 是ES6 中新增的用来比较两个值 和  === 基本一致

4、重点是 === 和 Object.is 的关系 
  === 比较好说 一般遵守以下的规则就可以：
  * 1、类型不同就不相等
  * 2、如果两个是数值类型 数值相等就是相等
...
### UDP 和 TCP 的区别:
  UDP 全称是用户数据报协议，位于传输层，主要的作用是处理数据包，是一种无连接的协议。

  UDP 协议有以下特点：
  1、面向无连接：
    首先UDP 不需要三次握手那样的操作，就可以直接发送数据，并且是数据的搬运工，不会对数据报文做任何的拆分和拼接操作。
  2、有单播、多播、广播
    UDP 不止支持一对一的传输方式，同样支持一对多，多对多，多对一的方式，也就是说 UDP 提供了单播，多播，广播的功能。
  3、不可靠性
    首先不可靠性体现在无连接上，通信都不需要建立连接，想发就发，这样的情况肯定不可靠。并且收到什么数据就传递什么数据，并且也不会备份数据，发送数据也不会关心对方是否已经正确接收到数据了。
  4、头部开销小, 发送的效率会高一些，没有拥塞机制，无论网路情况如何，传输的速率都是一样的，容易造成丢包的情况。
  ...
  4、可靠传输
    TCP为了保证数据的传输可靠, 会给数据包添加标记序号,这样能够保证发送和接收的可靠性。

### 请问你是如何理解前端模块化的？
  前端模块化就是复杂的文件变成一个个独立的模块，分离成单独的模块有助于重用（复用性）和维护（迭代性）
  这样就有了 模块之间的依赖问题，因此就有了 commonjs规范，AMD CMD 以及打包编译等问题。

  将一个复杂的程序 按照一定的规则封装成几个块，并组合在一起
  块的内部数据实现是私有的，只是想外部暴露一些接口。与其他模块进行通信。


### 请你谈谈DNS的寻址过程：
  1、操作系统会先检查自己本地的hosts文件是否有这个网址映射关系，如果有，就先调用这个IP地址映射，完成域名解析。 
  2、如果没有本机的host映射，就在本地计算机缓存查找，若没有找到，就会将请求发送给dns服务器
  3、先发送给本地dns服务器，本地的就会在自己的区域里面查找，若找到，根据此记录进行解析。
...
### 什么是防抖，你能简单的手写实现一下吗？
  1、我们在前端开发中会遇到一些频繁触发的时间，例如 window.resize 或者 mousemove 或者输入框不断地输入内容然后调取接口试算
  查询等等。频发触发如果回调响应不及时，会造成卡顿，因此我们需要防抖处理。

  防抖原理就是: 你尽管触发事件，但是我一定在时间触发n秒后才执行，如果你在时间触发n秒内又触发了这个事件，那我就以新的事件的时间为准，n 秒后才执行，总之，就是要等你触发完事件 n 秒内不再触发事件，我才执行。
...
+ 简单的自我介绍与项目经验
+ 一道算法题目
+ 一道setTimeout/Promises输出顺序的问题
+ 什么是事件循环
+ react 中的key 用来做什么？
+ 什么是虚拟DOM
+ webpack的原理是什么，loader 和 plugin 的作用是什么？
+ ssr 的原理是什么，解决了什么问题
+ ssr 会有哪些坑
+ 你如何看待前端和后端
+ 有没有了解过shell
...
+ docker的底层原理是什么
+ 有没有了解过ufs
+ 部署node时候如何充分利用服务器的多核
+ 如果你要读取一个特别大文件应该如何做
+ 你们有没有做对服务端的异常监控
+ 你们在线上出现问题如何在应用层面监控cpu和memory信息
+ 如何查看一个node的服务器应用的内存和CPU
+ 当服务器出现OOM的问题如何解决
+ 数据库中什么是left join 什么是 right join
+ 数据库索引为什么要使用Btree
+ redis 如何做持久化
+ 如何实现一个分布式锁
+ 有没有用火LUA
+ 当一个地址从输入到展示在浏览器中有哪些步骤
+ 递归解析的过程是什么样的，什么叫做递归DNS和权威DNS
+ TCP为什么是可靠的
+ TCP 相比UDP的话，udp有什么好处，虽然不可靠，但是为什么还有很多基于udp协议
